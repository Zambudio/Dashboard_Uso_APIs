...
/** Abre la ventana de login en el proceso principal de Electron y captura la cookie de sesión. */
async function captureSessionCookie(broker, payload, fetchImpl = fetch) {
  const res = await fetchImpl(`${broker.url}/session/capture`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${broker.token}`, 'Content-Type': 'application/json' },
  ...
  if (!res.ok) {
    ...
    throw new Error(`No se pudo capturar la cookie de sesión (HTTP ${res.status}): ${text.slice(0, 200)}`);
  }
  const json = await res.json();
  return json.data;
}

/** Establece cookies en la sesión de Electron y hace fetch de una lista de URLs autenticadas. */
async function fetchSessionUrls(broker, payload, fetchImpl = fetch) {
  const res = await fetchImpl(`${broker.url}/session/fetch`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${broker.token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
    cache: 'no-store',
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => '');
    throw new Error(`No se pudo consultar la sesión autenticada (HTTP ${res.status}): ${text.slice(0, 200)}`);
  }
  const json = await res.json();
  if (!json.ok) throw new Error(json.error || 'Error consultando la sesión autenticada');
  return json.data;
}
