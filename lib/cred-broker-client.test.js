...
const { resolveBrokerConfig, readKeysFromBroker, writeKeysToBroker, readConfigFromBroker, writeConfigToBroker } = require('./cred-broker-client');
...
test('readKeysFromBroker disables the Next.js fetch cache so writes are never read back stale', async () => {
  const calls = [];
  const fakeFetch = async (url, opts) => {
    calls.push({ url, opts });
    return { ok: true, json: async () => ({}) };
  };
  await readKeysFromBroker({ url: 'http://127.0.0.1:1234', token: 'tok' }, fakeFetch);
...
test('config broker helpers use /config with no-store semantics', async () => {
  const calls = [];
  const fakeFetch = async (url, options) => {
    calls.push({ url, options });
    return { ok: true, json: async () => ({ providers: [] }) };
  };
  const broker = { url: 'http://127.0.0.1:1234', token: 'tok' };
  assert.deepEqual(await readConfigFromBroker(broker, fakeFetch), { providers: [] });
  await writeConfigToBroker(broker, { preferences: {} }, fakeFetch);
  assert.equal(calls[0].url, `${broker.url}/config`);
