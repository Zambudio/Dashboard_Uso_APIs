    ...
    <form onSubmit={handleSubmit} className="rounded-2xl border border-white/10 bg-[#151521] p-5 shadow-xl shadow-black/20">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400">La clave se guarda en el .env local del servidor, no en el navegador.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="space-y-2 text-sm text-slate-300">
          ...
          <input
            ...
            placeholder="Por ejemplo: Azure OpenAI"
            required
          />
        </label>
      ...
      <label className="mt-4 block space-y-2 text-sm text-slate-300">
        <span>{definition.secretLabel}</span>
        <input
          value={apiKey}
      ...
      <div className="mt-5 flex justify-end">
        <button type="submit" className="rounded-xl bg-gradient-to-r from-fuchsia-500 to-cyan-500 px-4 py-2 font-medium text-white transition hover:opacity-90">
          Guardar integración
        </button>
      </div>
    </form>
