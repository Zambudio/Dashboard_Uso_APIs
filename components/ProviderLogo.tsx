...
export function ProviderLogo({ provider, size = 'md' }: ProviderLogoProps) {
  ...
  if (imageSrc && !imageFailed) {
    return (
      <span
        className={`flex shrink-0 items-center justify-center overflow-hidden ${badge} bg-black/30 shadow-inner shadow-black/20`}
      >
  ...
  const gradient =
    provider === 'gemini'
      ? { background: 'linear-gradient(135deg, #4285F4 0%, #9B72CB 55%, #D96570 100%)' }
      : undefined;

  return (
    <span
      className={`flex shrink-0 items-center justify-center ${badge} ${gradient ? '' : bg} shadow-inner shadow-black/20`}
      style={gradient}
    >
