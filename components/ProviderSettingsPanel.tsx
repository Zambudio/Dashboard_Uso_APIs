...
export function ProviderSettingsPanel({
  ...
  const handleDelete = () => {
    if (
      window.confirm(
        `¿Eliminar la tarjeta "${provider.name}"? Se borrará también la clave o sesión guardada. Esta acción no se puede deshacer.`
      )
    ) {
    ...
    <div className="rounded-2xl border border-white/10 bg-[#13131a] p-5 shadow-2xl shadow-black/30">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-xs text-slate-300">
                Abre {definition.label} en una ventana de Chromium, haz login y extraeremos tus datos de consumo
                automáticamente.
              </p>
            </div>
            <button
      ...
      <label className="mt-4 block space-y-2 text-sm text-slate-300">
        ...
        <input
          ...
          className="w-full rounded-xl border border-white/10 bg-[#1f1f2b] px-3 py-2 text-white"
          placeholder={
            hasApiKey ? 'Guardada de forma cifrada — escribe para reemplazarla' : definition.secretPlaceholder
          }
        />
      </label>
      ...
      <div className="mt-3 rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-slate-400">
        <p>
          La credencial se guarda cifrada con la protección del sistema operativo. El navegador solo conoce si está
          configurada, nunca su valor.
        </p>
      </div>
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        {onDelete ? (
          <button
            ...
            className="rounded-xl border border-rose-500/30 bg-rose-500/10 px-4 py-2 text-sm font-medium text-rose-300 transition hover:bg-rose-500/20"
          >
            Eliminar tarjeta
          </button>
        ) : (
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => onSave(form)}
            className={`rounded-xl px-4 py-2 text-sm font-medium text-white ${statusTone} bg-white/10`}
          >
            Guardar cambios
          </button>
        </div>
